...
def validate_fields(email, password):
    if email is None or password is None or not isinstance(email, str) or not isinstance(password, str):
        return jsonify({"message": "Correo y contraseña requeridos"}), 400

    if re.sub(r"[a-zA-Z][\w.]*@\w+\.[\w.]+", "", email) != "":
        return jsonify({"message": "Correo no es válido"}), 400

    if len(password) < 4:
        return jsonify({"message": "La contraseña debe tener una longitud de 4 carácteres"}), 400
...
def logout():
    ...
    with mysql.get_db().cursor() as cursor:
        cursor.execute("UPDATE users SET session_expires = NOW() - INTERVAL 30 MINUTE WHERE user_id = %s;", (user_id,))
    mysql.get_db().commit()

    return jsonify({"message": "Sesión terminada"}), 200
